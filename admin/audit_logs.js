import { success, badRequest, forbidden, serverError, CORS_HEADERS } from "../../utils/response.js";
...
export async function listAuditLogs(request, env) {
    ...
    if (!hasViewPerm) {
        return forbidden("Access denied. Insufficient permissions to view security audit logs.");
    }
...
export async function exportAuditLogs(request, env) {
    ...
    if (!hasViewPerm) {
        return forbidden("Access denied. Insufficient permissions to export audit logs.");
    }
